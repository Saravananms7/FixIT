...
const MyIssues = () => {
  ...
  const fetchMyIssues = async () => {
    try {
      setLoading(true);
      // Filter by postedBy to get only user's issues
      const params = {
        ...filters,
        postedBy: user?._id,
  ...
  const getIssueStats = () => {
    ...
    const inProgress = issues.filter(issue => issue.status === 'in_progress' || issue.status === 'assigned').length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-gray-600">Manage your submitted support issues</p>
        </div>
        <Link
          to="/issues/new"
          ...
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {filters.search || filters.status || filters.priority || filters.category
                ...
                : activeTab === 'resolved' ? 'You have not resolved any issues yet' : 'You haven\'t submitted any issues yet'}
            </p>
